...
async function refresh(){try{const s=await json('/api/status');$('version').textContent='v'+s.version;$('defaultPw').hidden=!s.default_password;const pairs=[['IP',s.ip],['mDNS',s.mdns],['Wi-Fi RSSI',s.rssi+' dBm'],['Uptime',s.uptime],['Free heap',s.free_heap],['Free PSRAM',s.free_psram],['HA',s.ha_status],['OTA slot',s.ota_partition]];$('stats').innerHTML=pairs.map(p=>`<div class="stat"><div class="k">${esc(p[0])}</div><div class="v">${esc(p[1])}</div></div>`).join('')}catch(e){}}
async function loadCfg(){const c=await json('/api/config');$('hostname').value=c.hostname;$('username').value=c.username;$('backlight').value=c.backlight;$('dark').value=c.dark?'1':'0';$('timeout').value=String(c.timeout);$('chore_entity').value=c.chore_entity||'';$('alarm_entity').value=c.alarm_entity||''}
$('cfg').addEventListener('submit',async e=>{e.preventDefault();$('cfgMsg').textContent='Saving…';const p=new URLSearchParams();for(const id of ['hostname','username','password','backlight','dark','timeout','chore_entity','alarm_entity'])p.set(id,$(id).value);try{await json('/api/config',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:p});$('cfgMsg').textContent='Saved. Reboot to apply.';$('password').value=''}catch(e){$('cfgMsg').textContent=e.message}});
